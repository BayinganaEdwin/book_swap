...
class _MyListingsScreenState extends State<MyListingsScreen> {
      ...
      children: [
        ...
        Expanded(
          child: _tabIndex == 0
              ...
              : _tabIndex == 1
                  ? _buildSwapOffersList(auth.user!.uid, theme, isReceived: true)
                  : _tabIndex == 2
                      ? _buildSwapOffersList(auth.user!.uid, theme, isReceived: false)
                      : _buildSwappedBooksList(bookProv, auth.user!.uid, theme),
        ),
      ],
    );
  }
  ...
  Widget _buildSwapOffersList(String uid, ThemeData theme, {required bool isReceived}) {
    final firestore = FirestoreService();

    return StreamBuilder<List<Map<String, dynamic>>>(
      stream: isReceived 
          ? firestore.streamReceivedSwapOffers(uid)
          : firestore.streamSentSwapOffers(uid),
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          return Center(
            child: Column(
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            return _SwapOfferCard(offer: offers[index], isReceived: isReceived);
          },
        );
      },
    );
  ...
  Widget _buildSwappedBooksList(BookProvider bookProv, String uid, ThemeData theme) {
    return StreamBuilder<List<Book>>(
      stream: bookProv.mySwappedBooks(uid),
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          return Center(
            child: Column(
        ...
        if (books.isEmpty) {
          return Center(
            child: Column(
              ...
              children: [
                Icon(
                  _searchQuery.isNotEmpty ? Icons.search_off : Icons.swap_horiz_rounded,
                  size: 80,
                  color: theme.colorScheme.onSurface.withValues(alpha: 0.3),
                ),
                const SizedBox(height: 16),
                Text(
                  _searchQuery.isNotEmpty ? 'No books found' : 'No swapped books',
                  style: TextStyle(
                    fontSize: 20,
                    fontWeight: FontWeight.w600,
                    color: theme.colorScheme.onSurface,
                ...
                Padding(
                  ...
                  child: Text(
                    _searchQuery.isNotEmpty
                        ? 'Try a different search term'
                        : 'Books you\'ve swapped will appear here',
                    style: TextStyle(
                      fontSize: 14,
                      color: theme.colorScheme.onSurface.withValues(alpha: 0.6),
                    ),
...
class _SwapOfferCard extends StatelessWidget {
  ...
  const _SwapOfferCard({required this.offer, this.isReceived = true});
    ...
    return FutureBuilder<Book?>(
      ...
      builder: (context, bookSnapshot) {
        ...
        return FutureBuilder<String?>(
          future: firestore.getUserName(isReceived ? (offer['from'] ?? '') : (offer['to'] ?? '')),
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  isReceived ? 'Wants to swap' : 'Swap status',
                                  style: TextStyle(
                                    fontSize: 14,
                                    color: theme.colorScheme.onSurface.withValues(alpha: 0.6),
                                  ),
                      ...
                      if (book != null) ...[
                        ...
                        if (offer['offeredBookId'] != null) ...[
                          FutureBuilder<Book?>(
                            future: firestore.getBook(offer['offeredBookId']),
                            builder: (context, offeredBookSnapshot) {
                              final offeredBook = offeredBookSnapshot.data;
                              if (offeredBook == null) return const SizedBox.shrink();
  ...
  Future<void> _handleAccept(BuildContext context, FirestoreService firestore) async {
    ...
    final offeredBookId = offer['offeredBookId'] ?? '';
    
    if (offerId.isEmpty || bookId.isEmpty || offeredBookId.isEmpty) {
    ...
    try {
      await firestore.acceptSwapOffer(offerId, bookId, offeredBookId);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  Future<void> _handleDecline(BuildContext context, FirestoreService firestore) async {
    ...
    if (confirmed == true) {
      try {
        await firestore.declineSwapOffer(offerId, bookId, offeredBookId);
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
